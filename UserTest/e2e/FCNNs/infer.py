...
def build_layer5_net(in_size, l1_size, l2_size, l3_size, l4_size, out_size):
    keep_prob = tf.placeholder(tf.float32, name='keep_prob')
    xs = tf.placeholder(tf.float32, [None, in_size], name='xs')
    ...
    L0 = add_layer(xs, in_size, l1_size, activation_function=tf.nn.relu, name='L0')
    L1 = add_layer(L0, l1_size, l2_size, activation_function=tf.nn.relu, name='L1')
    ...
    prediction = add_layer(L3, l4_size, out_size, activation_function=None, name='prediction')
...
def build_layer7_net(in_size, l1_size, l2_size, l3_size, l4_size, l5_size,l6_size, out_size):
    keep_prob = tf.placeholder(tf.float32, name='keep_prob')
    xs = tf.placeholder(tf.float32, [None, in_size], name='xs')
    ys = tf.placeholder(tf.float32, [None, out_size], name='ys')
    sess = tf.Session()
    ...
    prediction = add_layer(L5, l6_size, out_size, activation_function=None, name='prediction')
...
def evaluation(batch_size,in_size,out_size):
    ...
    with tf.Session() as sess:
        ...
        valiation_x_1 = np.random.uniform(0,1,[batch_size,in_size])
        ...
        #the first run is warmup and will be dropped

        s1 = time.time()
        prediction_y = sess.run(y, feed_dict={xs: valiation_x_1, ys: valiation_y, keep_prob:1.0})
        e1 = time.time()
        warmup_time = ((e1 - s1)) * 1000
        print('warmup time(ms):', warmup_time)
        ...
        for num in range(0, 10):
            ...
            t_list.append((e1-s1)*1000)

        avg_t = np.average(t_list)
        print('avg time(ms):', avg_t)
        warmup_time_array = np.array([warmup_time])

        line = np.hstack((warmup_time_array,avg_t))


        my_arr = np.loadtxt("batch="+str(batch_size)+".txt")
        new_arr = np.vstack((my_arr,line))

        np.savetxt("batch="+str(batch_size)+".txt",new_arr)
...
if __name__ == "__main__":
    ...
    in_size, l1_size, l2_size, l3_size, l4_size, out_size = 128, 128 ,256 ,512, 1024 ,1000
    ...
    build_layer7_net(in_size, l1_size, l2_size, l3_size, l4_size,l5_size, l6_size, out_size)
    evaluation(batch_size,in_size,out_size)


    #evaluation(1, in_size, out_size)
    #evaluation(16, in_size, out_size)
    #evaluation(32, in_size,  out_size)
    #evaluation(64, in_size, out_size)
    #evaluation(128, in_size,  out_size)
    #evaluation(256, in_size, out_size)
    #evaluation(512, in_size,  out_size)
