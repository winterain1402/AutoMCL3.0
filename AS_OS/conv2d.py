...
"""Conv2D schedule on OptSchedule"""
'''conv2d base'''
import logging
import re
...
#  nn.conv2d  --- conv2d
@autotvm.register_topi_compute(nn.conv2d, 'cpu', ['direct'])
...
def _declaration_conv_impl(cfg, data, kernel, strides, padding, dilation, layout, out_dtype):
    ...
    #ic_bn: input_channel tiling size
    #oc_bn: output_channel tiling size 
    #"tile_ow" unused for now 
    ...
    '''
    n： batch size
    oc_chunk: num_filter//oc_bn
    oh: out_height
    ow：out_width
    oc_block: oc_bn(tiling size for output channel)
...
def schedule_conv2d(cfg, outs):
    ...
    def traverse(op):
        #print("schedule_conv2d_nchw traverse")
        ...
        if 'conv2d_nchw' in op.tag:
            ...
            #print("conv_out.op.input_tensors:",conv_out.op.input_tensors)
            kernel = kernel_vec.op.input_tensors[0]
            #print("kernel_vec.op.input_tensors:",kernel_vec.op.input_tensors)
...
def _schedule_conv2d_NCHWc(cfg, outs):
    #print("schedule conv2d nchwc")
    """Create schedule for tensors"""
    s = tvm.create_schedule([x.op for x in outs])
    scheduled_ops = []

    def traverse(op):
        #print("schedule_conv2d_NCHWc traverse")
        """Traverse operators from computation graph"""
        # inline all one-to-one-mapping operators except the last stage (output)
        if tag.is_broadcast(op.tag):
            if op not in s.outputs:
