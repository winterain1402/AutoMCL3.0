...
def get_gap8s(n):
    p = 0
    if p<1:
        res = [2**x for x in range(math.floor(math.log2(n)) + 1)]
        p+=1
    if p>=1:
        res1 = [8 *(x+1) for x in range(math.floor(n // 8))]
        p+=1
    res.extend(res1)
    res = list(set(res))
    res.sort()
...
def get_gap8s_v2(n):
    res = [1]
    res1 = [8*(x+1) for x in range(math.floor(64//8))]
    res2 = [2**x for x in range(math.floor(math.log2(n))+1)]
    res.extend(res1)
    res.extend(res2)
    res = list(set(res))
    res.sort()
...
def get_oracle(M,pThread=8):
    ...
    for p1 in P1_set:
        for i in range(math.ceil(M / p1)):
            ...
            #if by < M:
               # candidateTM.add(by + 1)
    candidateTM = sorted(candidateTM)
    return list(candidateTM)
    ...
    def __init__(self, axes, policy, **kwargs):
        ...
        if policy == 'candidate':
            ...
            if policy == 'verbose':
                ...
                factors = [x for x in get_gap8s(self.product) if x <= max_factor]
            elif policy == 'oracle':
                factors = [x for x in get_oracle(self.product) if x <= max_factor]
            else:
